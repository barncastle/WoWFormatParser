...
namespace WoWFormatParser.Helpers
{
    ...
    internal static class MPQSorter
    {
        public static int Sort(string a1, string a2)
        {
            ...
            if (isLocalePatch1)
            {
                bool hasNum1 = archive1.ExtIndex >= 1 && char.IsDigit(archive1.PatchNum[archive1.ExtIndex - 1]);
                bool hasNum2 = archive2.ExtIndex >= 1 && char.IsDigit(archive2.PatchNum[archive2.ExtIndex - 1]);
                if (hasNum1 != hasNum2)
                    return hasNum1 ? -1 : 1;
            }
